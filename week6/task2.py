"""Week 6 Task 2: predict Titanic survival status from sex and age."""
...
def encode_data(
    ...
    """Encode Sex and standardized Age as inputs, and Survived as expects."""
    ...
    for passenger in passengers:
        ...
        input_values.append([passenger.sex, age_standardizer.encode(age)])
...
def main() -> None:
    ...
    input_values, expected_values = encode_data(
        passengers, missing_age, age_standardizer
    )

    # Build neural network (2 inputs -> ReLU hidden layer -> Sigmoid output)
    nn = Network(
        layers=[
            Layer(
                neurons=[
